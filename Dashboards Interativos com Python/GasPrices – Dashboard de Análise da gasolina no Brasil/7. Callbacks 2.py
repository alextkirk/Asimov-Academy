...
def card2(data, estado, toggle):
    ...
    fig.add_trace(go.Indicator(
        mode='number+delta',
        title= {'text': f'<span style="size:60%">{estado}</span style="font-size:0.7em">{data1} - {data2}</span>'},
...
def graph1(data, ano, regiao, toggle):
    ...
    fig2_text = [f'{x} - R${y}' for x, y in zip(dff_estado.ESTADO.unique(), dff_regiao['VALOR REVENDA (R$/L)'].unique())]
    ...
    fig2.update_layout(xaxis_range=[dff_estado['VALOR REVENDA (R$/L)'].min() -0.15, dff_regiao['VALOR REVENDA (R$/L)'].max()])
...
def func(data, est1, est2, toggle):
    ...
    # Definindo o texto
    text = f'Comaparando {est1} e {est2}. Se a linha estiver acima do eixo X, {est2} tinha menor preço, do contrário, {est1} tinha um valor inferior'
    return [fig, text]
